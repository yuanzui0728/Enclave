...
## WeFlow 证据窗口补充（2026-04-27）

`weflow-http` 现在不再只提供联系人基础信息和少量消息样本，而是支持：
...
1. 全局默认：在后台步骤 1 的 `WeFlow 分析窗口`
2. 单人覆盖：在后台步骤 2 的 `单个联系人分析覆盖`

设计原则：

- 连接器尽量抓取完整证据
- Prompt、预览返回和导入快照只保留代表性样本
- 通过 `evidenceWindow` 元数据记录“请求了多少 / 实际抓到了多少 / 是否纳入朋友圈”
