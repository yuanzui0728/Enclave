...
- 桌面朋友圈、视频号、游戏、小程序这几条页面层工作区现在也会先归一 `pathname` 再判断桌面路由，`/tabs/moments/`、`/tabs/channels/`、`/tabs/games/`、`/tabs/mini-programs/` 这类尾斜杠历史地址会继续被页面自愈回标准 `/tabs/*`；桌面强提醒和提醒浮条也同步把 `/tabs/chat/` 视作当前消息工作区，不再因为尾斜杠把当前会话误判成“未打开”。
- 桌面壳现在也会把尾斜杠的桌面入口页和独立窗口页视作原同一路由，比如 `/welcome/`、`/desktop/chat-window/`、`/desktop/note-window/`。之前这些地址在壳层会被误判成普通主窗口，可能导致壳子按钮和窗口级行为错位。
- 桌面消息、搜索、通讯录、发现、收藏、资料这几条主工作区根页现在也会先归一 `pathname` 再做 `/tabs/*` 自愈和活跃态判断，`/tabs/chat/`、`/tabs/search/`、`/tabs/contacts/`、`/tabs/discover/`、`/tabs/favorites/`、`/tabs/profile/` 这类尾斜杠地址会直接收回标准桌面工作区，不再被页面层误当成旧路径。
...
- SPA 导航状态现在写入 session 前也会先收掉 pathname 尾斜杠。之前桌面入口已经能把 `/tabs/chat/`、`/tabs/feed/` 这类地址 replace 回标准路径，但导航状态里可能仍把尾斜杠旧地址记成上一页，后续 `navigateBackOrFallback` 会把用户又带回刚自愈过的 URL；现在浏览器回退兜底读取到的 current/previous path 也保持同一套标准路径。
