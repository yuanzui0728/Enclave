...
- `desktop/mobile` 里的旧资料/设置接力现在也会统一归进“快捷入口”，包括 `/profile`、`/tabs/profile`、`/profile/settings`、`/desktop/settings` 和 `/legal/*`，即使老记录里还带着 query 或 hash 也一样。之前这类历史一旦挂了 `#...` 或 `?...`，仍可能继续掉进“其他”，看起来像是不属于全局快捷入口的杂项链路。
