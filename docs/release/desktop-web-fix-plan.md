...
- 桌面搜索页和桌面搜索启动器现在会把 `/tabs/search` 返回状态统一补到角色资料、好友朋友圈、朋友圈、广场动态、视频号、游戏、小程序，以及 `发起群聊` 这类桌面结果上。之前桌面搜索只给角色资料补了返回协议，这几类结果打开后再点“返回上一页”或关闭弹窗，经常只能回各自工作区，带不回当前搜索条件；现在会直接回当前桌面搜索工作区，并保住关键词与来源上下文。
