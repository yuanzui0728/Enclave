...
- 控制台 `/sessions` 这轮也补了同样的 compact permalink：`admin-sessions-route-search` 新增了 compact builder 和 `/sessions?...` permalink helper，页面本身现在支持 `Copy sessions permalink` 和 `Open sessions permalink`，而顶栏 `Sessions` 导航也已经切到紧凑 `href`，不再把默认 session/source-group 过滤参数整串挂在地址栏上。
