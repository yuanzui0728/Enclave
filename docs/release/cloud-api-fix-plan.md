...
- 控制台 `/waiting-sync` 现在除了 failed task 的单条/批量 replay、clear 之外，也支持可见 `context` 聚合卡片、上下文聚焦、按 task target 的快速聚焦、过滤快照与 context/focus 快照的 JSON/CSV 导出、当前页 `context groups` summary 的 JSON/CSV 导出（含关联 `taskIds/taskKeys/targetValues`）、context 本地 review 面板（先看当前页关联 tasks，再决定是否切主过滤器）、把 review panel 本身持久化到 `reviewContext` / `reviewTaskId` route search 的 deep link、review 面板里的 `Copy review context` / `Copy task context`（附当前 `/waiting-sync` 紧凑 permalink 和目标 task permalink）以及 `Open review permalink` / `Open task permalink`、单条 task `Replay now` / `Clear now` 完成后的最近操作回执（含 `request-id`、task key、context、target 和 task permalink，clear 后 task 已不在当前页时也会继续保留在 review 面板里）、`refresh_world` 任务直达 world detail，以及手机号类任务直接跳到带 `query` 的 `/requests` 或 `/worlds` 检索页，方便从补偿任务继续反查关联申请单和世界。
