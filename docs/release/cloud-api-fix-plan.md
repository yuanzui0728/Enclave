...
- 控制台 `/waiting-sync` 里的手机号反查入口这轮也统一切到了 compact permalink link：`Open requests` / `Open worlds` 现在会直接渲染紧凑的 `/requests?query=...` / `/worlds?query=...`，不再把默认空参数一起拼进 `href`，同时点击后仍保持 router 内部导航。
