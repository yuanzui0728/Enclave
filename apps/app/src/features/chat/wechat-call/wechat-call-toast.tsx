...
export function WeChatCallToast({
  ...
  return (
    ...
    >
      <div className="pointer-events-auto flex max-w-[88%] items-center gap-3 rounded-2xl bg-black/72 px-4 py-2.5 text-[length:var(--text-caption)] leading-5 text-[color:var(--text-on-brand)] shadow-lg backdrop-blur">
        <span className={tone === "danger" ? "text-[color:var(--state-danger-text)]" : "text-[color:var(--text-on-brand)]/90"}>
          {message}
        </span>
        {action}
      </div>
