  ...
  return (
    <section className="rounded-2xl border border-[color:var(--border-faint)]/60 bg-[color:var(--surface-card)]/75 p-3 shadow-md backdrop-blur-md">
      <header className="mb-2 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-[color:var(--text-secondary)]">{t(msg`事件流`)}</h2>
        <span className="text-[length:var(--text-eyebrow)] text-[color:var(--text-dim)]">
          {eventsQuery.data?.length ?? 0} {t(msg`条`)}
        </span>
      </header>
      {eventsQuery.isLoading && (
        <p className="py-4 text-center text-xs text-[color:var(--text-dim)]">{t(msg`加载中……`)}</p>
      )}
      {eventsQuery.error && (
      ...
      <ul className="max-h-72 space-y-1 overflow-y-auto text-[length:var(--text-eyebrow)] text-[color:var(--text-muted)]">
        {eventsQuery.data?.map((event) => (
          <li
            key={event.id}
            className="flex items-baseline gap-2 rounded-md bg-[color:var(--surface-soft)] px-2 py-1"
          >
