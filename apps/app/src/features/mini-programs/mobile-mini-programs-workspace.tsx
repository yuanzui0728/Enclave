  ...
  return (
    <AppPage className="space-y-5">
      <TabPageTopBar
        eyebrow="Discover"
        title="小程序"
        subtitle="按微信式发现页节奏组织最近使用、我的小程序和推荐位"
        titleAlign="center"
        leftActions={
          <Button
            onClick={onBack}
        ...
        <div className="mt-3 space-y-3">
          <label className="relative block">
            ...
            <input
              ...
              className="h-11 w-full rounded-[16px] border border-transparent bg-white/88 pl-10 pr-12 text-sm text-[color:var(--text-primary)] outline-none transition-[background-color,border-color] placeholder:text-[color:var(--text-dim)] focus:border-[color:var(--border-faint)] focus:bg-white"
            />
            {searchText ? (
              <button
                type="button"
      ...
      <AppSection className="space-y-3 bg-white/92">
        <div className="text-sm font-medium text-[color:var(--text-primary)]">
          今日推荐
        </div>
        <div className="space-y-3">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? "border-[rgba(255,138,61,0.18)] bg-[rgba(255,245,235,0.92)]"
