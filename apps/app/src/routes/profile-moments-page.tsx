  ...
  const commentMutation = useMutation({
    onMutate: async (momentId: string) => {
      ...
      const tempComment: MomentComment = {
        ...
        // moments-page.tsx L759 / friend-moments-page / mobile-friend-moments-page
        // 同款修法 —— `??` 只 catch null/undefined；ownerName 为 "" / "   " 时
        // optimistic comment 的 authorName 落地为空，wechat-moment-card 渲染评论行
        // 变成 "：评论内容"（前面没人）。`?.trim() ||` 兜下空字符串。
        authorName: ownerName?.trim() || t(msg`我`),
            ...
            <div className="px-4 pt-12">
              <EmptyState
                ...
                action={
                  <Button
                    variant="primary"
                    className="rounded-full bg-[color:var(--brand-primary)] px-5 text-[13px] text-[#3b2206] shadow-none hover:bg-[color:var(--brand-primary)]"
                    onClick={goPublish}
                  >
                    {t(msg`发条朋友圈`)}
                  </Button>
