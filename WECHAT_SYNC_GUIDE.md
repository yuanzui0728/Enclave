...
> 已经按“多平台标准化导入层”方向推进。现阶段真实可用的平台仍然只有微信，
> QQ / Telegram / Discord 等属于下一阶段扩展目标。
...
| 手工 `WechatSyncContactBundle[]` JSON | 已落地 | 适合作为脚本转换或中间导出的兜底入口。 |
| QQ | 标准层已规划 | 计划优先接入导出文件，再映射到统一联系人模型。 |
| Telegram | 标准层已规划 | 计划优先接入导出文件，再映射到统一联系人模型。 |
| Discord | 标准层已规划 | 计划优先接入导出文件，再映射到统一联系人模型。 |
...
1. 不依赖 `5678`
2. 不依赖 `5031`
3. 只要你手头已有 `WechatSyncContactBundle[]` 格式的联系人快照 JSON
