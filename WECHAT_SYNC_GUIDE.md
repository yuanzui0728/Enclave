...
3. WeFlow 启动按钮当前是拉起本地 clone 的开发版应用；如果依赖没安装，仍需要先在对应目录执行 `npm install`
4. WeFlow 的 HTTP API 是否真正可用，仍取决于应用内是否已经开启 API 服务
5. 当前链路是“从联系人资料读取到隐界”，实时链路目前仍主要面向微信，不是“反向控制真实微信”
6. 不会主动给真实微信好友或群聊发消息
7. WeFlow 的朋友圈、媒体、群成员等更丰富能力当前还没有接进导入链路
...
## WeFlow 深度分析窗口（2026-04-27）

`WeFlow API` 模式下，角色预览现在会纳入更完整的证据，而不再只看少量聊天样本。
...
实现上会抓取更完整证据，但不会把全部原始历史直接塞回预览页或导入快照；预览响应、Prompt 上下文和快照只保留代表性样本，以及 `evidenceWindow` 元数据。
