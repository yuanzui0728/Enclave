...
function parseChatLabJsonl(raw: string): ParsedWechatSyncContactBundleResult {
  const lines = raw
    .split(/\r?\n/u)
    .map((line) => line.trim())
...
function parseChatLabExport(value: unknown): WechatSyncContactBundle {
  ...
  const contactMessageCount = messages.filter(
    (item) => item.direction === "contact" || item.direction === "group_member",
  ).length;
...
function normalizeBundleEntry(
  ...
  const nickname = readText(value.nickname) ?? readText(value.nickName) ?? null;
  const remarkName =
    readText(value.remarkName) ?? readText(value.remark) ?? null;
  const displayName =
  ...
  const latestMessageAt =
    normalizeTimestamp(value.latestMessageAt) ??
  ...
  return {
    username:
      platform !== "wechat"
        ? buildImportedUsername(platform, rawUsername)
        : rawUsername,
    displayName,
    nickname,
    remarkName,
...
function normalizeChatLabMessages(
  ...
  return value
    .map((entry) => {
      ...
      const typeLabel =
        typeValue != null ? CHATLAB_MESSAGE_LABELS.get(typeValue) : null;
      const text =
        readText(record.content) ?? (typeLabel ? `[${typeLabel}]` : null);
      if (!text) {
        return null;
      }
      ...
      return {
        ...
        senderLabel,
        timestamp:
          normalizeTimestamp(record.timestamp) ?? new Date(0).toISOString(),
        text,
        typeLabel: typeLabel ?? null,
        direction: normalizeChatLabDirection(
          typeValue,
          senderId,
          ownerId,
          isGroup,
...
function resolvePrivateIdentity(
  ...
  const displayName =
    member?.groupNickname ?? member?.accountName ?? name ?? platformId;
  ...
  for (const entry of value) {
    ...
    normalized.push({
      ...
      direction: normalizeDirection(
        readText(entry.direction) as ContactImportMessageDirection | null,
      ),
    });
  }
...
function normalizeMomentHighlights(
  value: unknown,
): WechatSyncMomentHighlight[] {
  if (!Array.isArray(value)) {
    return [];
...
function normalizeDirection(
  value: ContactImportMessageDirection | null,
): WechatSyncMessageDirection {
  switch (value) {
    case "owner":
    case "contact":
...
function compareBundles(
  left: WechatSyncContactBundle,
  right: WechatSyncContactBundle,
) {
...
function pickPreferredLabel(left: string, right: string, fallback: string) {
  if (!left) {
    return right;
  }
  if (!right) {
...
function normalizeStringList(value: unknown) {
  ...
  return [
    ...new Set(value.map((item) => readText(item) ?? "").filter(Boolean)),
  ];
}
