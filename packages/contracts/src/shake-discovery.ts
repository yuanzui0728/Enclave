...
export interface ShakeDiscoveryConfig {
  enabled: boolean;
  ...
  // 2026-05-27：非会员最多可保留的"摇一摇来源"好友数（character.sourceType==='shake_generated'
  // 且 friendship 仍有效）。达到上限后非会员发起/保留摇一摇被拦，需删除或开通会员。
  // 0 = 不限制。会员（status==='active' 或 hardBlock 关闭）不受此限制。
  freeFriendLimit: number;
  // 2026-05-22：planning 阶段是否调 web_search（token-plan /v1/coding_plan/search）
  // 注入实时热点 signal。默认 false 避免每次摇一摇都烧 200/日 配额。
...
export interface ShakeDiscoveryPreviewCharacter
  extends Pick<
    CharacterDraft,
    "name" | "avatar" | "relationship" | "expertDomains"
  > {
  relationshipType: CharacterDraft["relationshipType"];
}

export interface ShakeDiscoverySessionPreview {
  id: string;
  status: ShakeDiscoverySessionStatus;
  character: ShakeDiscoveryPreviewCharacter;
  greeting: string;
  matchReason: string;
  createdAt: string;
...
export interface KeepShakeDiscoverySessionResult {
  sessionId: string;
  status: "kept";
  characterId: string;
...
export interface DismissShakeDiscoverySessionResult {
  sessionId: string;
  status: "dismissed" | "expired";
}

export interface ShakeDiscoverySessionRecord
  extends ShakeDiscoverySessionPreview {
...
export interface ShakeDiscoveryStats {
  totalSessions: number;
  previewReadySessions: number;
  keptSessions: number;
...
export interface ShakeDiscoveryOverview {
  config: ShakeDiscoveryConfig;
  stats: ShakeDiscoveryStats;
  recentSessions: ShakeDiscoverySessionRecord[];
  activeSession: ShakeDiscoverySessionPreview | null;
}
