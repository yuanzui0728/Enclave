...
export class SocialService {
  ...
  async ensureDefaultFriendships(userId?: string): Promise<void> {
    const ownerId = userId ?? (await this.worldOwnerService.getOwnerOrThrow()).id;
    ...
    for (const characterId of DEFAULT_CHARACTER_IDS) {
      ...
      const existing = await this.friendshipRepo.findOneBy({ userId: ownerId, characterId });
      if (!existing) {
