...
// world 侧验证：私有角色视频注入只能由 cloud-api 用 CLOUD_SERVICE_TOKEN 发起。
// world 只绑 loopback，公网经 cloud-api 终结；这层是纵深防御，挡伪造的本地回环命中。
// 与 cloud-api 的 ServiceTokenGuard 对称（方向相反），同 MatchmakingServiceTokenGuard。
@Injectable()
export class FeedInternalServiceTokenGuard implements CanActivate {
