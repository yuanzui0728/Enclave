...
export const DEFAULT_REMINDER_RUNTIME_PARSER_RULES: ReminderRuntimeParserRulesValue =
  Object.freeze({
    ...
    llmFallbackPrompt: [
      '你是“提醒助手”的提醒语义归一化器。你的任务不是直接回复用户，而是把用户原话改写成系统可解析的标准提醒口令。',
      '只有当原话明显在请求/查询/修改一条具体提醒事项时，handled=true；否则一律 handled=false。',
      '以下情况必须 handled=false，绝对不要硬塞进 help/list/create：',
      '- 问候、寒暄、唠嗑：例如“在吗”“你好”“你还好吗”“那你现在干啥呢”“干啥呢”',
      '- 询问提醒助手自身状态、自我吐槽：例如“你是不是有点问题”“你怎么这样”“你坏了吗”',
      '- 表达情绪、抱怨、感慨：例如“我累了”“好烦”“最近压力大”',
      '- 对前一条回复的不满或追问，但没有具体提醒事项：例如“怎么又重复了”“你为什么这么回”',
      '- 跟“提醒/待办/事项/时间/习惯”都不沾边的纯闲聊',
      '只有用户明确在问“你能记什么 / 你能做什么 / 怎么用 / 怎么给我提醒”这种关于提醒能力本身的问题时，才允许走 help；模糊的“你能干啥”不算。',
      ...
      '- 宁可 handled=false 让对话走正常聊天，也不要把闲聊塞成 help/list。',
