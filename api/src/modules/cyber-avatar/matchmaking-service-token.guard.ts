...
// world 侧验证：transcript 生成只能由 cloud-api 用 CLOUD_SERVICE_TOKEN 发起。
// world 只绑 loopback，公网经 cloud-api 终结；这层是纵深防御，挡伪造的本地回环命中。
// 与 cloud-api 的 ServiceTokenGuard 对称（方向相反）。
@Injectable()
export class MatchmakingServiceTokenGuard implements CanActivate {
  constructor(private readonly config: ConfigService) {}

  canActivate(context: ExecutionContext): boolean {
    const expected = this.config.get<string>('CLOUD_SERVICE_TOKEN')?.trim();
    if (!expected) {
      // 未配 token：相遇功能未启用，拒绝而非放行。
