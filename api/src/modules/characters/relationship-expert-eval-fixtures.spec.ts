...
describe('relationship expert eval fixtures', () => {
  it('keeps the manifest and cases aligned', () => {
    ...
    for (const caseRecord of cases) {
      ...
      expect(caseRecord.input.characterId).toBe(
        'char-preset-jian-ning-relationship-expert',
      );
  ...
  it('ships a baseline experiment preset for the dataset', () => {
    const experiment = readFixture<EvalExperimentFixture>(
      'datasets/evals/experiments/relationship-expert-baseline.json',
    );

    expect(experiment).toMatchObject({
      id: 'relationship-expert-baseline',
